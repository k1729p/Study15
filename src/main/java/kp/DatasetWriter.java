...
public class DatasetWriter {
	...
	private void writeDataArray(Set<Map.Entry<String, Map<Path, Map<Integer, Set<String>>>>> datasetEntrySet,
		...
		datasetEntrySet.forEach(//
				projectEntry -> projectEntry.getValue().entrySet().forEach(//
						pathEntry -> pathEntry.getValue().entrySet().forEach(//
								lineEntry -> lineEntry.getValue().forEach(//
										keyword -> writeDataElement(//
												jsonGenerator, idAtomic, //
												projectEntry.getKey(), pathEntry.getKey(), //
												lineEntry.getKey(), keyword)//
								)//
						)//
				)//
		);
